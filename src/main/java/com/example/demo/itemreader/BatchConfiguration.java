...
public class BatchConfiguration {
	...
	@Bean
	@StepScope
	public FlatFileItemReader<Customer> customerItemReader(@Value("#{jobParameters['customFile']}") Resource inputFile)
	{
		return new FlatFileItemReaderBuilder<Customer>().name("customerItemReader")
//				.resource(inputFile)
				.resource(new FileSystemResource("C:\\Temp\\customerFixedWidth.txt"))
				.fixedLength()
				...
				.names(new String[] { "firstName", "middleInitial", "lastName", "addressNumber", "street", "city",
						"state", "zipCode" })
	...
	public Step copyFileStep() {
		return this.stepBuilderFactory.get("copyFileStep").<Customer, Customer>chunk(10)
				.reader(customerItemReader(null)).writer(itemWriter()).build();
	}

	@Bean
	public Job copyFileJob() {
		return this.jobBuilderFactory.get("copyFileJob").start(copyFileStep()).build();
	}
}
