...
//    @Bean
//    @StepScope
//    public JpaCursorItemReader<Customer> jpaCursorItemReader(EntityManagerFactory entityManagerFactory,   @Value("#{jobParameters['city']}") String city){
...
//    }
    ...
    @Bean
    @StepScope
    public RepositoryItemReader<Customer> repositoryItemReader(CustomerRepository customerRepository, @Value("#{jobParameters['city']}") String city){
    ...
    public Step copyFileStep() {
        return this.stepBuilderFactory.get("copyDBStep").<Customer, Customer>chunk(10)
                .reader(repositoryItemReader(null, null)).writer(itemWriter()).build();
    }
